...
*/
    ...
    i2c_data[0] = ROW_START >> 8; // MS byte of row start
    i2c_data[1] = ROW_START & 0xff; // LS byte
    i2c_master_write_reg(DEV_ADDR, ROW_START_REG, i2c_data, 2, i2c_ports);
    delay_milliseconds(1);

    i2c_data[0] = COL_START >> 8; // MS byte of col start
    i2c_data[1] = COL_START & 0xff; // LS byte
    i2c_master_write_reg(DEV_ADDR, COL_START_REG, i2c_data, 2, i2c_ports);
    delay_milliseconds(1);
...
void color_interpolation(chanend c_dc, unsigned frBuf){
    ...
    for (unsigned i=2; i<LCD_HEIGHT; i++){
        unsigned row = i-1;
